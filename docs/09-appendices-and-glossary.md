...
| :---------------------- | :----------- | :---------------------------- | :-------------------------- | :----------------------------------------------------------------- |
| `home`                  | Top-Level    | `/hu`                         | `/en`                       | [`HomePage.tsx`](../src/components/pages/HomePage.tsx)             |
| `ai-solutions`          | Flagship Hub | `/hu/ai-megoldasok`           | `/en/ai-solutions`          | [`ServicePage.tsx`](../src/components/pages/ServicePage.tsx)       |
| `business-systems`      | Flagship Hub | `/hu/uzleti-rendszerek`       | `/en/business-systems`      | [`ServicePage.tsx`](../src/components/pages/ServicePage.tsx)       |
| `data-forecasting`      | Flagship Hub | `/hu/adat-es-elorejelzes`     | `/en/data-forecasting`      | [`ServicePage.tsx`](../src/components/pages/ServicePage.tsx)       |
| `solutions`             | Catalogue    | `/hu/megoldasok`              | `/en/solutions`             | [`SolutionsPage.tsx`](../src/components/pages/SolutionsPage.tsx)   |
| `references`            | Evidence     | `/hu/referenciak`             | `/en/references`            | [`ReferencesPage.tsx`](../src/components/pages/ReferencesPage.tsx) |
| `how-we-work`           | Process      | `/hu/hogyan-dolgozunk`        | `/en/how-we-work`           | [`ProcessPage.tsx`](../src/components/pages/ProcessPage.tsx)       |
| `software-integrations` | Supporting   | `/hu/szoftver-es-integraciok` | `/en/software-integrations` | [`ServicePage.tsx`](../src/components/pages/ServicePage.tsx)       |
| `devops-infrastructure` | Supporting   | `/hu/devops-infrastruktura`   | `/en/devops-infrastructure` | [`SubPage.tsx`](../src/components/pages/SubPage.tsx)               |
| `about`                 | Corporate    | `/hu/rolunk`                  | `/en/about`                 | [`SubPage.tsx`](../src/components/pages/SubPage.tsx)               |
| `contact`               | Utility      | `/hu/kapcsolat`               | `/en/contact`               | [`SubPage.tsx`](../src/components/pages/SubPage.tsx)               |
...
| :------------------------------------------------------- | :---------------------------------------------------------------------- | :---------------------------------------------------------------------------------------------------------------------------------------------- |
| **`404 Page Not Found` on valid URL**                    | Slugs in URL belong to different language or mismatched family owner.   | Verify URL parameters match [`src/config/routes.ts`](../src/config/routes.ts) and [`src/config/solutions.ts`](../src/config/solutions.ts).      |
| **Theme flashing to white on page refresh in dark mode** | Inline script in `__root.tsx` was modified or removed.                  | Ensure [`themeInitScript`](../src/lib/theme.ts#L36-L46) is present in the `<head>` of [`src/routes/__root.tsx`](../src/routes/__root.tsx#L126). |
| **Search modal returns no results for existing text**    | Query normalization mismatch or missing index group.                    | Check [`buildSearchIndex`](../src/lib/search-index.ts#L37-L113) to ensure target content is registered in index groups.                         |
| **TypeScript compile error on `content.pages[key]`**     | Key missing from one of the language dictionaries (`hu.ts` or `en.ts`). | Check [`src/content/types.ts`](../src/content/types.ts) and ensure both `hu.ts` and `en.ts` provide matching keys.                              |
